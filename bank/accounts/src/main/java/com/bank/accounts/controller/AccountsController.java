...
public class AccountsController {
    ...
    @ApiResponses({
            ...
            @ApiResponse(
                    responseCode = "500",
                    description = "HTTP status Internal Server Error",
                    content = @Content( // 스웨거에서 500에러가 발생하여야 나오기 때문에 500에러가 발생하지 않더라도 화면에 보여주기 위해 설정
                            schema = @Schema(implementation = ErrorResponseDto.class)
                    )
            )
    }
    ...
    public ResponseEntity<ResponseDto> deleteAccountDetails(
        ...
        if(isDeleted) {
            ...
            return ResponseEntity
                    .status(HttpStatus.EXPECTATION_FAILED)
                    .body(new ResponseDto(AccountsConstants.STATUS_200, AccountsConstants.MESSAGE_417_DELETE));
        }
    }
...
//        throw new NullPointerException();
    ...
    @Operation(
            ...
            description = "Contact Info details that is deployed into accounts microservice"
    )
    @ApiResponses({
            @ApiResponse(
                    responseCode = "200",
